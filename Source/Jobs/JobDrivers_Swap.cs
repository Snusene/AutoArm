    ...
    public class JobDriver_SwapSidearm : JobDriver
    {
        ...
        protected override IEnumerable<Toil> MakeNewToils()
        {
            ...
            yield return Toils_Goto.GotoThing(TargetIndex.A, PathEndMode.ClosestTouch)
                ...
                .FailOn(() => NewWeapon.IsForbidden(pawn));
        ...
        private void PerformAtomicSidearmSwap(Pawn pawn, ThingWithComps newWeapon, ThingWithComps oldWeapon)
        {
            ...
            bool ssPreNotified = false;
            if (SimpleSidearmsCompat.IsLoaded && !SimpleSidearmsCompat.ReflectionFailed)
            {
                SimpleSidearmsCompat.InformOfDroppedWeapon(pawn, oldWeapon);
                ssPreNotified = true;
            }
            ...
            if (!isSameTypeUpgrade && SimpleSidearmsCompat.IsLoaded && !SimpleSidearmsCompat.ReflectionFailed)
            {
                string validationReason;
                if (!SimpleSidearmsCompat.CanPickupSidearm(newWeapon, pawn, out validationReason))
                {
                    AutoArmLogger.Debug(() => $"[{pawn.LabelShort}] Swap aborted - SS would reject {newWeapon.Label}: {validationReason}");
                    return;
                }
            }
            else if (isSameTypeUpgrade && AutoArmMod.settings?.debugLogging == true)
            {
            ...
            if (!pawn.inventory.innerContainer.TryAdd(newWeapon))
            {
                ...
                if (SimpleSidearmsCompat.IsLoaded && !SimpleSidearmsCompat.ReflectionFailed && ssPreNotified)
                {
                    SimpleSidearmsCompat.InformOfAddedSidearm(pawn, oldWeaponToPlace);
                }
