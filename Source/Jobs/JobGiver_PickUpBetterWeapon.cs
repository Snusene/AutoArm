...
namespace AutoArm.Jobs
{
    ...
    public class JobGiver_PickUpBetterWeapon : ThinkNode_JobGiver
    {
        ...
        // Lookups for TickScheduler event handling
        private static readonly Dictionary<int, (int pawnId, string messageType)> hashToKeyLookup =
            new Dictionary<int, (int pawnId, string messageType)>();
        ...
        // Encode key for TickScheduler (combine pawnId and messageType hash)
        ...
        private static bool ShouldLogDebugMessage(Pawn pawn, string messageType, string messageContent)
        {
            ...
            int encodedKey = EncodeMessageKey(pawnId, messageType);
            ...
            // Schedule expiry via TickScheduler
            int expireTick = currentTick + MESSAGE_SUPPRESSION_WINDOW;
            hashToKeyLookup[encodedKey] = key;
        ...
        /// <summary>
        /// Event handler called by TickScheduler when message cache entry expires
        /// </summary>
        public static void OnMessageCacheExpiredEvent(int encodedKey, int unused)
        {
            if (!hashToKeyLookup.TryGetValue(encodedKey, out var key))
                return;

            messageCache.Remove(key);
            hashToKeyLookup.Remove(encodedKey);
        }
        ...
        /// <summary>
        /// Safety check - expiry now handled by TickScheduler
        /// </summary>
        public static void CleanupMessageCache()
        {
            // Expiry handled by TickScheduler.ProcessTick() -> OnMessageCacheExpiredEvent
            // Just keep safety check for runaway cache
            if (messageCache.Count > 400)
            {
                messageCache.Clear();
                hashToKeyLookup.Clear();
            }
        }
        ...
        public Job TestTryGiveJob(Pawn pawn)
        {
                ...
                bestWeapon == null && AutoArmMod.settings?.autoEquipSidearms == true)
            {
                ...
                {
                    // FindBestSidearm returned null - pawn may be at SS limit
                    // but could still UPGRADE existing sidearms (swap doesn't change count)
        ...
        public ThingWithComps FindBestWeapon(Pawn pawn, float currentScore, ThingDef restrictToType, bool isForcedUpgrade, out string primaryFailureReason)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (restrictToType != null)
                    {
                        // Only build filtered list for type-restricted searches (forced upgrades)
                        filteredSearchSet = ListPool<Thing>.Get();
                    ...
                    {
                        // No pre-filtering needed - iterate directly with cap in main loop
                        // This avoids O(n) iteration just to build a list
                ...
                // Use per-tick cached reservation set from map component
                var mapComp = JobGiverMapComponent.GetComponent(pawn.Map);
                var reservedThings = mapComp?.GetReservedThings(currentTick) ?? reservationSetPool;
                ...
                // Cap rough candidates to limit cold-cache spike - 60 is enough to find good weapons
                int maxRoughCandidates = isUnarmed ? 80 : 60;

                // Hard iteration cap - bounds worst-case regardless of rejection rate
                ...
                if (searchSet != null)
                {
                    foreach (Thing thing in searchSet)
                    {
                        ...
                        // Size check moved here from pre-filter loop (avoids O(n) pre-iteration)
                        if (!IsValidSize(pawn, weapon))
                            continue;
                        ...
                        // Stop: enough candidates OR hit hard iteration cap
                    ...
                    foreach (var item in roughQueue)
                    {
                        ...
                        // Cap full evaluations to limit cold-cache spike
                        // Unarmed pawns get more evaluations (emergency), armed get fewer
        ...
        public ThingWithComps FindBestSidearm(Pawn pawn, out string failureReason)
        {
            ...
            // Build set of existing weapon defs (primary + inventory) for O(1) duplicate checks
            ...
            foreach (var weapon in candidateWeapons)
            {
                ...
                // Skip if pawn already has same-def weapon (primary or inventory)
                // Prevents duplicates - upgrades handled by TryFindSidearmUpgrade
        ...
        /// <summary>
        /// Find sidearm upgrade when at SS limit (bypasses CanPickupSidearm)
        /// </summary>
        private Job TryFindSidearmUpgrade(Pawn pawn)
        {
            ...
            // Get existing sidearm defs for quick lookup
            ...
            IEnumerable<ThingWithComps> weapons;
            if (storageOnly && outfit != null)
                weapons = WeaponCacheManager.GetStorageWeapons(pawn.Map, outfit);
            else if (storageOnly)
                weapons = WeaponCacheManager.GetAllStorageWeapons(pawn.Map);
            else if (outfit != null)
                weapons = WeaponCacheManager.GetWeaponsForOutfit(pawn.Map, outfit);
            else
                weapons = WeaponCacheManager.GetAllWeapons(pawn.Map);

            Job bestUpgradeJob = null;
            float bestScore = 0f;
            ...
            foreach (var weapon in weapons)
            {
                ...
                // Only consider weapons that match existing sidearm defs (same-def upgrades)
                if (!existingSidearmDefs.Contains(weapon.def))
                    continue;
                ...
                if (score > bestScore)
                {
                    // TryGetWeaponJob will check if this is actually an upgrade
        ...
        public static void CleanupCaches()
        {
            ...
            foreach (var map in Find.Maps)
            {
                ...
                // Skip all iterations if all caches are empty (common on new games)
                ...
                if (comp.ValidationCache.Count > 0)
                {
                    ...
                    // LRU eviction without sorting - single pass O(n) instead of O(n log n)
                    if (comp.ValidationCache.Count > maxCacheSize)
                    {
                        ...
                        foreach (var kvp in comp.ValidationCache)
                        {
                            int tick = kvp.Value.ExpiryTick;
                            if (tick < minTick) minTick = tick;
                            if (tick > maxTick) maxTick = tick;
                        }

                        int cutoffTick = minTick + (maxTick - minTick) / 4;
                        var toEvict = ListPool<PawnWeaponKey>.Get(entriesToRemove);
                        foreach (var kvp in comp.ValidationCache)
                        {
                            if (kvp.Value.ExpiryTick <= cutoffTick)
                            {
                                toEvict.Add(kvp.Key);
                                if (toEvict.Count >= entriesToRemove) break;
                            }
                        }
                        ...
                        int removedCount = toEvict.Count;
                        ListPool<PawnWeaponKey>.Return(toEvict);
                ...
                // PawnStates cleanup - find dead pawns and reset stale equip ticks
                if (comp.PawnStates.Count > 0)
                {
                    ...
                    {
                        if (Cleanup.IsPawnInvalid(kvp.Key))
                        {
                            deadPawns.Add(kvp.Key);
                        }
    ...
    public class JobGiverMapComponent : MapComponent
    {
        ...
        // Per-tick cached reservation set - avoids rebuilding for every pawn
        [Unsaved]
