...
namespace AutoArm
{
    ...
    public static class Thing_LabelPatches
    {
        ...
        {
            // Cheapest check first - skip non-weapons immediately
            if (!__instance.def.IsWeapon)
                return true;

            // Tick-cached check (ShouldProcessWeaponLabel handles settings check)
        ...
        public static int CleanupLabelCache()
        {
            ...
            var toRemove = new System.Collections.Generic.List<int>(cache.Count / 4);
