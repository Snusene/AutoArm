    ...
    public static class ChildWeaponPatches
    {
        ...
        private static void CanEquip_Postfix(Thing thing, Pawn pawn, ref bool __result, ref string cantReason)
        {
            if (!__result && thing != null && thing.def.IsWeapon && pawn != null)
            {
                ...
                bool isAllowedChild = false;
                if (settings?.allowChildrenToEquipWeapons == true)
                {
                    if (minAge <= 13)
                    {
                        isAllowedChild = pawn.DevelopmentalStage < DevelopmentalStage.Adult && age >= minAge;
                    }
                    else
                    {
                        isAllowedChild = age >= minAge && age < Constants.ChildMaxAgeLimit;
                    }
                }
