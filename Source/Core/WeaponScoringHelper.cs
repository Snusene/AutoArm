...
namespace AutoArm.Weapons
{
    ...
    public static class WeaponScoringHelper
    {
            ...
            else
            {
                ...
                if (pawn.skills?.skills != null)
                {
                    ...
                    else
                    {
                        ...
                        if (skillIndexCache.ContainsKey(pawnId))
                        {
                            // Cancel old schedule before rescheduling
                            TickScheduler.Cancel(TickScheduler.EventType.SkillCacheExpiry, pawnId);
                        }

                        skillIndexCache[pawnId] = (shootingIndex, meleeIndex, currentTick);

                        int indexExpireTick = currentTick + SkillCacheLifetimeTicks * 8;
                        TickScheduler.Schedule(indexExpireTick, TickScheduler.EventType.SkillCacheExpiry, pawnId);
                    }
                }
                else
                {
                ...
                if (skillCache.ContainsKey(pawnId))
                {
                    // Cancel old schedule before rescheduling
                    TickScheduler.Cancel(TickScheduler.EventType.SkillCacheExpiry, pawnId);
                }
                ...
                TickScheduler.Schedule(expireTick, TickScheduler.EventType.SkillCacheExpiry, pawnId);
            }
        ...
        /// <summary>
        /// Event handler called by TickScheduler when skill cache entry expires
        /// </summary>
        public static void OnSkillCacheExpiredEvent(int pawnId)
        {
        ...
        /// Now handled by TickScheduler.ProcessTick() -> OnSkillCacheExpiredEvent
        /// </summary>
        ...
        public static void RebuildSkillCacheSchedule()
        {
            ...
            foreach (var kvp in skillIndexCache)
            {
                ...
                if (expireTick > currentTick)
                {
                    // Only schedule if not already scheduled by skillCache loop
                    if (!skillCache.ContainsKey(pawnId))
                    {
                        TickScheduler.Schedule(expireTick, TickScheduler.EventType.SkillCacheExpiry, pawnId);
                        scheduledCount++;
