...
namespace AutoArm
{
    ...
    internal static class AutoArmLogger
    {
        ...
        private static readonly Regex IdParenPattern = new Regex(@"\(ID:\s*\d+\)", RegexOptions.Compiled);
        ...
        private static void InitializeLogFile()
        {
            try
            {
                ...
                if (File.Exists(path))
                {
                    ...
                    catch
                    {
                        try
                        {
                            ...
                            Writer.WriteLine();
                            Writer.WriteLine("===============================================================");
                            Writer.WriteLine($"AutoArm session resumed: {DateTime.Now:yyyy-MM-dd HH:mm:ss}");
                            Writer.WriteLine("===============================================================");
                            Writer.WriteLine();
                            return;
                        }
                ...
                Writer.WriteLine("===============================================================");
                Writer.WriteLine($"AutoArm session started: {DateTime.Now:yyyy-MM-dd HH:mm:ss}");
                Writer.WriteLine("===============================================================");
                Writer.WriteLine();
            ...
            {
                Writer = null;
                Verse.Log.Warning("[AutoArm] Could not create log file, debug logging disabled this session");
            }
        }
        ...
        { Verse.Log.Message("<color=#4287f5>[AutoArm]</color> " + message); }
        ...
        public static void InfoFileOnly(string message)
        {
            ...
            try
            {
                ...
                Writer.WriteLine($"{ts} {SanitizeMessage(message)}");
            }
            catch
            {
        ...
        public static void WarnFileOnly(string message)
        { Write(LogLevel.Warn, message, null, fileOnly: true); }
        ...
        public static void WarnCleanup(Exception ex, string cleanupType)
        {
            WarnFileOnly($"[Cleanup:{cleanupType}] {ex?.GetType().Name ?? "error"}: {ex?.Message ?? "unknown"}");
        }
        ...
        private static void Write(LogLevel level, string message, Exception ex = null, bool fileOnly = false)
        {
            string normalized;
            int bucket;
        ...
        private static string NormalizeMessage(string message, LogLevel level)
        {
            ...
            message = IdParenPattern.Replace(message, "(ID: X)");
