...
namespace AutoArm
{
    ...
    public class AutoArmGameComponent : GameComponent
    {
        ...
        public override void StartedNewGame()
        {
            ...
            // Map component cache built in FinalizeInit after maps fully exist
        ...
        public override void LoadedGame()
        {
            ...
            // Clear unified scheduler before rebuilding
            TickScheduler.Reset();

            // Rebuild schedules - these will reschedule via TickScheduler
        ...
        /// <summary>
        /// Called after game fully initialized - maps guaranteed to exist
        /// </summary>
        public override void FinalizeInit()
        {
            base.FinalizeInit();

            // Build map component cache here, not on first tick
            // FinalizeInit runs after maps are fully loaded
            RebuildMapComponentCache();

            // Pre-warm caches to avoid first-use spikes during gameplay
            WarmupCaches();
        }
        ...
        private void WarmupCaches()
        {
            ...
            // Warm SimpleSidearms reflection and caches first
            ...
            foreach (var map in maps)
            {
                ...
                // Get all weapons on this map once
                var weapons = Caching.WeaponCacheManager.GetAllWeapons(map);
                var weaponList = new List<ThingWithComps>();
                foreach (var w in weapons)
                    weaponList.Add(w);

                // Snapshot colonists - FreeColonists is live list
                ...
                foreach (var pawn in colonists)
                {
                    ...
                    Caching.PawnValidationCache.CanConsiderWeapons(pawn);

                    // Warm skill cache (melee + ranged)
                    ...
                    // Pre-warm actual weapon scores and eligibility for this pawn
                    foreach (var weapon in weaponList)
                    {
                        if (weapon != null && !weapon.Destroyed)
                        {
                            // Warm score cache
                            WeaponScoringHelper.GetTotalScore(pawn, weapon);

                            // Warm eligibility cache (EquipmentUtility.CanEquip)
                            Caching.EquipEligibilityCache.CanEquip(pawn, weapon, out _);

                            // Warm SimpleSidearms CanPickupSidearm cache
                            if (ssLoaded)
                            {
            ...
            // Start grace period - skip cleanup until caches could be stale
        ...
        // Cached map components - refreshed when map count changes
        private List<(Caching.WeaponCacheManager.AutoArmWeaponMapComponent cache, JobGiverMapComponent jobGiver)> cachedMapComponents;
        private int lastMapCount = -1;

        private void RebuildMapComponentCache()
        {
            var maps = Find.Maps;
            if (maps == null || maps.Count == 0)
            {
                cachedMapComponents = new List<(Caching.WeaponCacheManager.AutoArmWeaponMapComponent, JobGiverMapComponent)>(0);
                lastMapCount = 0;
                return;
            }

            cachedMapComponents = new List<(Caching.WeaponCacheManager.AutoArmWeaponMapComponent, JobGiverMapComponent)>(maps.Count);
            foreach (var map in maps)
            {
                if (map == null) continue;
                var cacheManager = map.GetComponent<Caching.WeaponCacheManager.AutoArmWeaponMapComponent>();
                var jobGiver = JobGiverMapComponent.GetComponent(map);
                cachedMapComponents.Add((cacheManager, jobGiver));
            }
            lastMapCount = maps.Count;
        }

        /// <summary>
        /// Tick cleanup - unified single dictionary lookup via TickScheduler
        /// </summary>
        public override void GameComponentTick()
        {
            ...
            // Single dictionary lookup for all tick-based events
            // Replaces 6+ separate dictionary lookups per tick
            TickScheduler.ProcessTick(currentTick);

            // Map component processing - use cached references
            // (Per-map events not yet migrated to TickScheduler)
            var maps = Find.Maps;
            if (maps != null && maps.Count > 0)
            {
                // Refresh cache if map count changed (rare - new map added/removed)
                if (cachedMapComponents == null || maps.Count != lastMapCount)
                {
                    RebuildMapComponentCache();
                }

                // Process using cached references - no GetComponent calls
                for (int i = 0; i < cachedMapComponents.Count; i++)
                {
                    var (cacheManager, jobGiver) = cachedMapComponents[i];
                    cacheManager?.ProcessExpiredReservations(currentTick);
                    jobGiver?.ProcessExpiredTempBlacklists(currentTick);
                }
            }
