    ...
    public static class WeaponCacheManager
    {
        ...
        public static IEnumerable<ThingWithComps> GetWeaponsForOutfit(Map map, ApparelPolicy outfit)
        {
            ...
            else
            {
                ...
                int outfitId = outfit.GetHashCode();
                ...
                foreach (var weapon in GetAllWeapons(map))
                {
                    ...
                    if (allowed && CheckQualityRequirements(weapon, outfit))
                    {
                        yield return weapon;
                    }
                }
        ...
        public static float GetCachedScore(Pawn pawn, ThingWithComps weapon)
        {
            ...
            {
                // Jitter TTL to prevent synchronized expiry spikes
                int jitter = ((pawnId * 31) ^ weaponId) % ScoreCacheJitterRange - (ScoreCacheJitterRange / 2);
        ...
        public static int CleanupScoreCache(bool forceDeadPawnCleanup = false)
        {
            ...
            if (shouldCleanupDeadPawns && scoreCache.Count > 0)
            {
                ...
                // HashSet lookup avoids O(n^2)
                var validPawnIds = new HashSet<int>();
            ...
            // Skip expiry check if nothing has been added recently
            ...
            if (scoreCache.Count > 400)
            {
                ...
                foreach (var pawnEntry in scoreCache)
                {
                    ...
                    if (mostRecentTick < minTick) minTick = mostRecentTick;
                    if (mostRecentTick > maxTick) maxTick = mostRecentTick;
                }

                // Evict oldest 25%
                int threshold = minTick + (maxTick - minTick) / 4;
                var toEvict = ListPool<int>.Get();
                foreach (var pawnEntry in scoreCache)
                {
                    int mostRecentTick = 0;
                    foreach (var scoreEntry in pawnEntry.Value.Values)
                    ...
                    if (mostRecentTick <= threshold)
                        toEvict.Add(pawnEntry.Key);
                }

                int evictedCount = 0;
                foreach (var pawnId in toEvict)
                {
                ...
                ListPool<int>.Return(toEvict);

                if (AutoArmMod.settings?.debugLogging == true && evictedCount > 0)
                {
                    AutoArmLogger.Debug(() => $"Evicted {evictedCount} stale pawn caches (threshold-based), now {scoreCache.Count}");
                }
            }
        ...
        {
            // Clear outfit filter cache for this outfit
            if (outfit != null)
            {
                int outfitId = outfit.GetHashCode();
