...
{
    internal sealed class UnarmedPawnTest : ITestScenario
    {
        public string Name => "Unarmed pawn picks up weapon";
        ...
        public void Setup(Map map)
        {
            ...
            WeaponCache.EnsureCacheExists(map);
            ...
            if (testPawn != null)
            {
                CleanupHelper.PreparePawnForTest(testPawn);
                ...
                if (weaponDef != null)
                {
                    ...
                    if (testWeapon != null)
                    {
                        ...
                        var cachedWeapons = WeaponCache.GetAllWeapons(map).ToList();
                        if (!cachedWeapons.Contains(testWeapon))
                        {
                            AutoArmLogger.Warn($"[TEST] UnarmedPawnTest: Created weapon not found in cache! Cache size: {cachedWeapons.Count}");
                            AutoArmLogger.Warn($"[TEST] Weapon spawned: {testWeapon.Spawned}, destroyed: {testWeapon.Destroyed}, map: {testWeapon.Map?.uniqueID}");
                        }
                        else
                        {
                        }
        ...
        public TestResult Run()
        {
            if (testPawn == null)
            {
                AutoArmLogger.Warn("[TEST] UnarmedPawnTest: Test pawn creation failed");
            ...
            {
                AutoArmLogger.Warn($"[TEST] UnarmedPawnTest: Pawn is not unarmed - expected: null weapon, got: {testPawn.equipment.Primary.Label}");
                return TestResult.Failure("Pawn is not unarmed");
            }

            if (testPawn.jobs == null || testPawn.equipment == null)
            {
                AutoArmLogger.Warn($"[TEST] UnarmedPawnTest: Pawn systems not initialized - jobs: {testPawn.jobs != null}, equipment: {testPawn.equipment != null}");
            ...
            if (job != null)
            {
                if (!JobValidator.ValidateJob(job, testPawn, out string validateReason))
                {
                    AutoArmLogger.Warn($"[TEST] UnarmedPawnTest: Job validation failed - {validateReason}");
            ...
            else
            {
                ...
                if (!isValidPawn)
                {
                    AutoArmLogger.Warn($"[TEST] UnarmedPawnTest: Pawn validation failed - reason: {reason}");
                }
                else if (testWeapon != null)
                {
                    // Production validation
                    bool passesValidation = jobGiver.ShouldConsiderWeapon(testPawn, testWeapon, testPawn.equipment?.Primary);
                    var score = WeaponCache.GetCachedScore(testPawn, testWeapon);
                    AutoArmLogger.Warn($"[TEST] UnarmedPawnTest: Available weapon {testWeapon.Label} - passesValidation: {passesValidation}, score: {score}");
                    reason = passesValidation ? "Valid but not chosen" : "Failed validation, enable debug logging";
                }
            }

            AutoArmLogger.Warn($"[TEST] UnarmedPawnTest: No weapon pickup job created - reason: {reason}");
            return TestResult.Failure($"No weapon pickup job created for unarmed pawn: {reason}");
        }

        public void Cleanup()
        {
            CleanupHelper.CleanupTest(testPawn, testWeapon);
            testWeapon = null;
            testPawn = null;
        }
    }

    internal sealed class WeaponUpgradeTest : ITestScenario
    {
        public string Name => "Weapon upgrades";
        private Pawn testPawn;
        ...
        public void Setup(Map map)
        {
            ...
            if (pistolDef != null && rifleDef != null)
            {
                ...
                if (betterWeapon != null)
                {
                    ...
                    var nearbyWeapons = WeaponCache.GetAllWeapons(map);
        ...
        public TestResult Run()
        {
            if (testPawn == null)
            {
                AutoArmLogger.Warn("[TEST] WeaponUpgradeTest: Test pawn is null");
            ...
            if (testPawn.equipment?.Primary == null)
            {
                AutoArmLogger.Warn($"[TEST] WeaponUpgradeTest: Pawn has no equipped weapon - equipment tracker exists: {testPawn.equipment != null}");
                return TestResult.Failure($"Pawn unarmed, equipment tracker: {testPawn.equipment != null}");
            }

            if (betterWeapon == null || !betterWeapon.Spawned)
            {
                AutoArmLogger.Warn($"[TEST] WeaponUpgradeTest: Better weapon null or not spawned - null: {betterWeapon == null}, spawned: {betterWeapon?.Spawned ?? false}");
                return TestResult.Failure($"Better weapon missing, null={betterWeapon == null}");
            }

            WeaponCache.EnsureCacheExists(testPawn.Map);
            if (!WeaponCache.IsWeaponTracked(testPawn.Map, betterWeapon))
            {
                WeaponCache.AddWeaponToCache(betterWeapon);
                WeaponCache.MarkCacheAsChanged(testPawn.Map);
            ...
            var currentScore = WeaponCache.GetCachedScore(testPawn, currentWeapon);
            var betterScore = WeaponCache.GetCachedScore(testPawn, betterWeapon);
            ...
            {
                AutoArmLogger.Warn($"[TEST] WeaponUpgradeTest: Better weapon score not high enough - expected: >{currentScore * TestConstants.WeaponUpgradeThreshold}, got: {betterScore} (current: {currentScore})");
                return TestResult.Failure($"Score too low: {betterScore} vs {currentScore * TestConstants.WeaponUpgradeThreshold} needed");
            }
            ...
            if (job == null)
            {
                ...
                bool passesValidation = jobGiver.ShouldConsiderWeapon(testPawn, betterWeapon, testPawn.equipment?.Primary);
                ...
                else if (!passesValidation) failureReason = "Weapon failed validation, enable debug logging";
                ...
                else failureReason = "Unknown, check JobGiver logic";

                AutoArmLogger.Warn($"[TEST] WeaponUpgradeTest: No upgrade job created - {failureReason}");
                AutoArmLogger.Warn($"[TEST] WeaponUpgradeTest: Current: {currentWeapon?.Label} (score: {currentScore:F1}), Better: {betterWeapon?.Label} (score: {betterScore:F1}) at distance {testPawn.Position.DistanceTo(betterWeapon.Position):F1}");
            ...
            if (!isValidEquipJob)
            {
                AutoArmLogger.Warn($"[TEST] WeaponUpgradeTest: Wrong job type - expected Equip/Swap, got: {jobName}");
                return TestResult.Failure($"Wrong job type: {jobName}");
            }

            if (job.targetA.Thing != betterWeapon)
            {
                AutoArmLogger.Warn($"[TEST] WeaponUpgradeTest: Job targets wrong weapon - expected: {betterWeapon?.Label}, got: {job.targetA.Thing?.Label}");
        ...
        public void Cleanup()
        {
            CleanupHelper.CleanupTest(testPawn, currentWeapon, betterWeapon);
            currentWeapon = null;
            betterWeapon = null;
            testPawn = null;
        }
    ...
    internal sealed class OutfitFilterTest : ITestScenario
    {
        public string Name => "Outfit filter restrictions";
        private Pawn testPawn;
        ...
        public void Setup(Map map)
        {
            ...
            if (rifleDef != null)
            {
                ...
                var masterworkRifle = CreateTestWeapon(map, rifleDef, pos + new IntVec3(4, 0, 0), QualityCategory.Masterwork, 0.3f);
                if (masterworkRifle != null)
                {
                    expectedWeapon = masterworkRifle;
                    weapons.Add(masterworkRifle);
                }
            }
        ...
        public TestResult Run()
        {
            if (CECompat.IsLoaded)
                return TestResult.Skip("CombatExtended changes ranged scoring; vanilla outcome doesn't apply");

            if (testPawn == null)
            {
                AutoArmLogger.Warn("[TEST] OutfitFilterTest: Test pawn creation failed");
                return TestResult.Failure("Test pawn creation failed");
            }
            ...
            foreach (var weapon in weapons)
            {
                ...
                if ((int)quality >= (int)QualityCategory.Good)
                    allowedCount++;
                else
                    rejectedByQuality++;
            }
            ...
            if (job != null && job.targetA.Thing is ThingWithComps chosenWeapon)
            {
                ...
                if ((int)chosenQuality < (int)QualityCategory.Good)
                    return TestResult.Failure($"Chose quality {chosenQuality}, filter requires Good+");

                if (expectedWeapon != null && chosenWeapon != expectedWeapon)
                    return TestResult.Failure($"Chose {chosenWeapon.Label}, expected {expectedWeapon.Label} (highest-scoring allowed)");
            }
            else if (allowedCount > 0)
            {
                return TestResult.Failure($"No job created, {allowedCount} weapons match filter");
            }
            else
            {
                result.Data["Note"] = "No weapons match filter, no job expected";
            }
        ...
        public void Cleanup()
        {
            ...
            CleanupHelper.DestroyWeapons(weapons);
            weapons.Clear();
            expectedWeapon = null;

            CleanupHelper.DestroyPawn(testPawn);
        }
    }
