...
namespace AutoArm.Testing.Scenarios
{
    ...
    public class TestSimpleSidearmsValidation : ITestScenario
    {
        ...
        private void TestUnarmedValidation(Dictionary<string, object> data)
        {
            ...
            {
                string reason;
                // Unarmed pawns picking up primary - SS shouldn't block this
                ...
                if (!canPickup)
                {
                    data[$"{key}_Reason"] = reason ?? "Unknown";
                    if (reason == null || (!reason.Contains("SimpleSidearms") && !reason.Contains("weight")))
                    {
                        AutoArmLogger.Warn($"[TEST] Unarmed pawn cannot pickup {weapon.Label}: {reason ?? "no reason given"}");
                    }
                }
            }
        }
