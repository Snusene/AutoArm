...
{
    internal sealed class RaceConditionTest : ITestScenario
    {
        ...
        public void Setup(Map map)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                if (pawn != null)
                {
                    ...
                    AutoArm.Testing.Helpers.CleanupHelper.PreparePawnForTest(pawn);
                }
            }
            ...
            if (targetWeapon != null)
            {
                ...
                WeaponCache.AddWeaponToCache(targetWeapon);

                WeaponCache.ClearScoreCache();

                var weaponsInCache = WeaponCache.GetAllWeapons(map)?.ToList() ?? new List<ThingWithComps>();
                ...
                {
                    AutoArmLogger.Warn($"[TEST] RaceConditionTest: Target weapon not in cache! Retrying cache rebuild...");
                    WeaponCache.AddWeaponToCache(targetWeapon);

                    weaponsInCache = WeaponCache.GetAllWeapons(map)?.ToList() ?? new List<ThingWithComps>();
                ...
                {
                    AutoArmLogger.Debug(() => $"[TEST] RaceConditionTest: Target weapon cached: {targetWeapon.Label} at {targetWeapon.Position}");
                }
            }
        }
        ...
        public TestResult Run()
        {
            ...
            var weaponsInCache = WeaponCache.GetAllWeapons(targetWeapon.Map)?.Count() ?? 0;
            ...
            if (weaponsInCache == 0)
            {
                return TestResult.Failure($"Cache empty, {weaponsOnMap} weapons on map but 0 cached");
            }
            ...
            foreach (var pawn in testPawns)
            {
                ...
                catch (Exception e)
                {
                    ...
                    AutoArmLogger.Warn($"[TEST] RaceCondition: Exception for {pawn.Name}: {e.Message}");
                }
            }
    ...
    internal sealed class WeaponDestructionMidJobTest : ITestScenario
    {
        public string Name => "Weapon destroyed mid-check";
        ...
        public TestResult Run()
        {
            ...
            if (testWeapons.Count > 1)
            {
                ...
                var weapons = WeaponCache.GetAllWeapons(testPawn.Map).ToList();
    ...
    internal sealed class InfiniteLoopTest : ITestScenario
    {
        public string Name => "No infinite loops";
        private Pawn testPawn;
        ...
        public void Setup(Map map)
        {
            ...
            if (testPawn != null)
            {
                ...
                if (weaponA != null)
                {
                    ...
                    WeaponCache.AddWeaponToCache(weaponA);
                }
                ...
                if (weaponB != null)
                {
                    ...
                    WeaponCache.AddWeaponToCache(weaponB);

                    var weaponsInCache = WeaponCache.GetAllWeapons(testPawn.Map).ToList();
                    if (!weaponsInCache.Contains(weaponA) || !weaponsInCache.Contains(weaponB))
                    {
                        AutoArmLogger.Warn($"[TEST] InfiniteLoopTest: Weapons not in cache! Cache has {weaponsInCache.Count} weapons");
                    }

                    if (weaponA != null && !weaponA.Spawned)
                    {
                        AutoArmLogger.Warn($"[TEST] InfiniteLoopTest: WeaponA not spawned!");
                    }
                    if (weaponB != null && !weaponB.Spawned)
                    {
                        AutoArmLogger.Warn($"[TEST] InfiniteLoopTest: WeaponB not spawned!");
                    }
                }
            }
        }
        ...
        public TestResult Run()
        {
            ...
            for (int i = 0; i < maxIterations; i++)
            {
                ...
                if (job != null && job.targetA.Thing is ThingWithComps weapon)
                {
                    ...
                    if (!otherWeapon.Spawned && !otherWeapon.Destroyed)
                    {
                        ...
                        WeaponCache.AddWeaponToCache(otherWeapon);
                    }
                }
                else
                {
            ...
            if (testPawn.equipment?.Primary != null)
            {
                ...
                if (dropped != null)
                {
                    DroppedItems.MarkAsDropped(dropped, 600);
